...
function nytimes() {
    ...
    function render(data) {
        ...
        data.results.map(article => {
            ...
            var img = $(`<img class='rounded mx-auto d-block' src='${article.multimedia[0].url}'>`);
            $(imgDiv).append(img);
            var a = $("<a target='_blank'class= 'card-link' href=" + article.url + ">" + article.title + "</a></br>");
            var p = $("<p class='card-text'  href=" + article.url + ">" + article.abstract + "</p></br>");
